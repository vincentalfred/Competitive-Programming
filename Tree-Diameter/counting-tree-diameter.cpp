...
int n, depth[N], dist[N], s[N], diameter;
...
int main() {
	...
	while (!q.empty()) {
		int u = q.front(); q.pop();
		if (dist[u] > (diameter+1)/2) break;
		
		if (!(diameter&1) && dist[u] == diameter/2 && adj[u].size() > 1) {
			center.pb(u);
			break;
		}
		else if (diameter&1 && ((dist[u] == diameter/2) || (dist[u] == (diameter+1)/2)) && adj[u].size() > 1) {
			center.pb(u);
		}
